	...
	peerConnection.OnTrack(func(track *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
		...
		go func() {
			for {
				...
				audioMutex.Lock()
				audioBuffer = append(audioBuffer, pkt.Payload...)
				audioMutex.Unlock()
			}
		}()
		...
		go func() {
			...
			for range ticker.C {
				...
				if len(audioBuffer) > 0 {
					...
					text, err := speechToText(bufferCopy)
					if err != nil {
...
func speechToText(audioData []byte) (string, error) {
	...
	fileWriter, err := writer.CreateFormFile("file", "audio.opus")
	if err != nil {
...
func sendAudioToTrack(track *webrtc.TrackLocalStaticSample, audioData []byte) error {
	...
	sample := media.Sample{
		Data:     audioData,
		Duration: time.Duration(len(audioData)/960) * frameDuration,
	}
